...
class map:
    ...
    def point_clearh(self,point,size):
        for i in range(size//2):
            for j in range(size//2):
                if i != 0:
                    self.img[point[0]+i][point[1]+j] = [0,0,0]
                    self.img[point[0]+i][point[1]-j] = [0,0,0]
                    self.img[point[0]-i][point[1]+j] = [0,0,0]
                    self.img[point[0]-i][point[1]-j] = [0,0,0]
                else:
                    self.img[point[0]+i][point[1]+j] = [255,255,255]
                    self.img[point[0]+i][point[1]-j] = [255,255,255]
                    self.img[point[0]-i][point[1]+j] = [255,255,255]
                    self.img[point[0]-i][point[1]-j] = [255,255,255]

    def point_clearv(self,point,size):
        for i in range(size//2):
            for j in range(size//2):
                if j != 0:
                    self.img[point[0]+i][point[1]+j] = [0,0,0]
    ...
    def update(self):
        ...
        if  np.random.randint(0,12) == 0:
            ...
            if (i == 0) or (i == 9):
                ...
                self.addcar([i*self.block_size + 10,j*self.block_size + 10],direct,hv)
            else:
        ...
        for index,i in enumerate(self.cars):
            ...
            if i.hv == 0:
                self.point_clearh(i.point,self.car_size)
            else:
                self.point_clearv(i.point,self.car_size)
    ...
    def addcar(self,position,dir,hv):
        self.cars.append(car(position,dir,hv))
